...
public:
	...
	LedStripUserSysexManager::LedStripUserSysexManager()
	{
		musicalPalette = RainbowColors_p;
	}

	boolean LedStripUserSysexManager::handleSysex(byte command, byte argc, byte* argv) {
		...
		if (command == STRIP_DATA)
		{
			...
			case DIGITAL_MUSICAL_PATTERN:
				if (argc != 13) return;
				c0 = CRGB(argv[1] | argv[2] << 7, argv[3] | argv[4] << 7, argv[5] | argv[6] << 7);
				c1 = CRGB(argv[7] | argv[8] << 7, argv[9] | argv[10] << 7, argv[11] | argv[12] << 7);
				SetupMusicalPalette();
				break;
	...
	boolean LedStripUserSysexManager::handlePinMode(byte pin, int mode)
	{
		return true;
	}

	void LedStripUserSysexManager::handleCapability(byte pin)
	{
	}

	void LedStripUserSysexManager::reset()
	{
	}

	void LedStripUserSysexManager::SetupMusicalPalette()
	{
		CRGB black = CRGB::Black;

		musicalPalette = CRGBPalette16(c0, black, c1, black, c0, black, c1, black, c0, black, c1, black, c0, black, c1, black);
	}

	void LedStripUserSysexManager::SetupBreathingPalette()
	{
...
void Musical_analog() 
{
	analogWrite(RED_PIN, 255-sysexManager.analogPalette.r);
	analogWrite(GREEN_PIN, 255-sysexManager.analogPalette.g);
	analogWrite(BLUE_PIN, 255-sysexManager.analogPalette.b);
} 
